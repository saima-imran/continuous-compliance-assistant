...
def _read_json(path):
    ...
    if not isinstance(data, list):
        raise DataValidationError(f"{file_path} must contain a JSON list.")
    return data
...
def _validate(records, required_fields, label):
    ...
    for index, record in enumerate(records, start=1):
        ...
        if record["id"] in seen:
            raise DataValidationError(f"Duplicate {label} ID: {record['id']}")
        seen.add(record["id"])

def load_requirements(path):
    records = _read_json(path)
    _validate(records, {"id","title","description","source","obligation_ids"}, "requirement")
    return [Requirement(**r) for r in records]
